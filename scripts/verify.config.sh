...
verify_checks() {
    cat <<'CHECKS'
py-syntax :: python -m py_compile install.py .claude/hooks/block_wide_fs_search.py .claude/hooks/tests/run_cases.py
shell-syntax :: for f in scripts/*.sh core/files/scripts/*.sh; do sh -n "$f" || exit 1; done
